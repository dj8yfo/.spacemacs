...
(defun ace-isearch-jump-during-isearch-helm-swoop ()
  ...
  (interactive)
  (if (not (< ace-isearch-input-length 0))
      (if (< (length isearch-string) ace-isearch-input-length)
          (cond ((eq ace-isearch--ace-jump-or-avy 'ace-jump)
                 (progn (isearch-exit)
                        (evil-scroll-line-to-center nil)
                        (if jump-after-helm-swoop (run-with-timer 0.05 nil 'ace-jump-last-search))))
                ((eq ace-isearch--ace-jump-or-avy 'avy)
                 (let ((avy-all-windows nil))
                   (avy-isearch))))
        (cond ((eq ace-isearch--ace-jump-or-avy 'ace-jump)
               (progn (if jump-after-helm-swoop (run-with-timer 0.05 nil 'ace-jump-last-search))
                      (setq isearch-string helm-swoop-pattern)
                      (helm-exit-minibuffer)))
              ((eq ace-isearch--ace-jump-or-avy 'avy)
               (progn
                 ;; (message (format "swoop: %s" helm-swoop-pattern))
                 (if jump-after-helm-swoop (run-with-timer 0.05 nil 'avy-jump-last-search))
                 (setq isearch-string helm-swoop-pattern)
                 (helm-exit-minibuffer)))))))
